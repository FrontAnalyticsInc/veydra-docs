...
<AccordionGroup>
  ...
  <Accordion icon="chalkboard" title="Canvas">
    ...
    **Step 2: Configure Placements (Enable Deep Linking)**
    1. Click **Edit** on the newly created key
    2. Set **Privacy Level** to **Public** (required for Veydra to receive user name and email)
    3. Scroll to **Placements** and add at least one:
    ...
    5. *(Optional)* For grade passback, scroll to **LTI Advantage Services** and enable:
    ...
    **Step 4: Register in Veydra**
    ...
    <Tip>
      After installation, instructors can add Veydra content by going to **Modules → + → External Tool → Veydra** or **Assignments → + Assignment → External Tool**. A content picker will appear showing available models.
    </Tip>
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion icon="chalkboard" title="Canvas (with Deep Linking)">
    ...
    3. A **content picker** opens showing available models from your Veydra account
    4. Select one or more models, click **Add Selected**
    5. The selected content is embedded as a link — students click it to launch directly into that model
...
<Tip>
  **Custom parameters vs Deep Linking**: You can target specific content by either:
  ...
  - Setting `lab_slug` or `model_id` as custom LTI parameters — works on any LMS
</Tip>
...
When configured for Deep Linking, Veydra displays an interactive content picker inside your LMS. This lets instructors visually browse and select which model or lab to embed.

### How Deep Linking Works

1. Instructor clicks **Add External Tool → Veydra** in the LMS
2. The LMS sends a `LtiDeepLinkingRequest` to Veydra
3. Veydra displays a content picker showing available models
4. Instructor selects one or more models and clicks **Add Selected**
5. Veydra sends a signed response back to the LMS with the selected content
6. The LMS creates a link/assignment pointing to that specific model
...
The content picker shows models belonging to:
1. The **Owner Filter** configured on the platform (if set)
2. Otherwise, the **user who registered** the LTI platform in Veydra

To change which models appear, edit your platform registration in **Account → LTI Integrations** and set the **Owner Filter** to the desired user ID.
